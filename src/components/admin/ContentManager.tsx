...
// ── TopicPanel ─────────────────────────────────────────────────
function TopicPanel({ topics, selected, onSelect, onAdd, onDelete, loading }: {
  ...
  async function handleAdd() {
    if (!name.trim()) return;
    setSaving(true);
    await onAdd(name.trim(), color, icon, desc.trim());
    setName(''); setDesc(''); setColor(COLORS[0]); setIcon(ICONS[0]);
    setShowForm(false);
    setSaving(false);
  }
  ...
  return (
    <div style={{ width: 260, flexShrink: 0, borderRight: `1px solid ${T.border}`, display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ padding: '16px 16px 12px', borderBottom: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => setShowForm(v => !v)} style={{ fontSize: 20, background: 'none', border: 'none', cursor: 'pointer', color: T.blue, lineHeight: 1 }}>+</button>
      </div>
      ...
      {showForm && (
        <div style={{ padding: '12px 14px', borderBottom: `1px solid ${T.border}`, background: '#F8FAFC' }}>
          ...
          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 10 }}>
            {ICONS.map(ic => (
              <button key={ic} onClick={() => setIcon(ic)} style={{ fontSize: 18, width: 32, height: 32, borderRadius: 6, border: icon === ic ? `2px solid ${T.blue}` : `1px solid ${T.border}`, background: icon === ic ? '#EFF6FF' : '#fff', cursor: 'pointer' }}>{ic}</button>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={handleAdd} disabled={!name.trim() || saving} style={{ flex: 1, padding: '8px', borderRadius: 8, border: 'none', background: T.blue, color: '#fff', fontWeight: 700, fontSize: 12, cursor: name.trim() && !saving ? 'pointer' : 'not-allowed', opacity: name.trim() && !saving ? 1 : 0.6, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {topics.map(t => (
          <div key={t._id} onClick={() => onSelect(t)}
            style={{ padding: '12px 14px', cursor: 'pointer', borderBottom: `1px solid ${T.border}`, background: selected?._id === t._id ? '#EFF6FF' : '#fff', display: 'flex', alignItems: 'center', gap: 10, transition: 'background 0.12s' }}>
            ...
            <button onClick={e => { e.stopPropagation(); onDelete(t._id); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#DC2626', fontSize: 16, padding: '4px', flexShrink: 0, opacity: 0.7, lineHeight: 1 }}>×</button>
          </div>
        ))}
      </div>
    </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {subtopics.map((s, idx) => (
          <div key={s._id} onClick={() => onSelect(s)}
            ...
            <button onClick={e => { e.stopPropagation(); onDelete(s._id); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#DC2626', fontSize: 16, padding: '4px', flexShrink: 0, opacity: 0.7, lineHeight: 1 }}>×</button>
          </div>
        ))}
      </div>
    </div>
